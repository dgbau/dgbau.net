...
export const middleware = (): RequestHandler => {
  if (appConfig.environment === 'test' || appConfig.environment === 'production') {
  ...
  morgan.token('code', (req, res) => (res && res.statusCode ? res.statusCode.toString() : '???'));

  morgan.token('pid', () => String(process.pid));

  morgan.format('local', (tokens, req, res) => getColorFn(res && res.statusCode)(tokens, req, res));
